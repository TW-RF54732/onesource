...
mod tests {
    ...
    fn test_profile_merging() {
        let mut args = Args {
            ...
            output_path: None,
            no_ignore: None,
            include: Some("cli_include".to_string()),
            exclude: None,
            tree_include: None,
            tree_exclude: None,
            no_tree: None,
        ...
        let mut config_doc = ConfigDocument {
            profiles: HashMap::new(),
        };
        config_doc.profiles.insert("backend".to_string(), ProfileConfig {
            include: Some("config_include".to_string()),
            exclude: Some("config_exclude".to_string()),
            max_size: Some(100),
            ..Default::default()
        });
        ...
        args.merge_saved_config(file.path());

        // CLI arg should win
        assert_eq!(args.include, Some("cli_include".to_string()));
        // Config should fill None
        assert_eq!(args.exclude, Some("config_exclude".to_string()));
        assert_eq!(args.max_size, Some(100));
    }
}